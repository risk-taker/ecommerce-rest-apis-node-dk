...
const productController = {
    ...
    async destroy(req, res, next) {
        ...
        // image delete
        const imagePath = document._doc.image; // document._doc => it gives document without any getters. 
    ...
    async index(req, res, next) {
        let documents;
        // pagination mongoose-pagination --> if you have large no. of products.
        try {
            documents = await Product.find()
                .select('-updatedAt -__v')
                .sort({ _id: -1 }); // --> all products get sort in descending order.
        } catch (err) {
            return next(CustomErrorHandler.serverError());
        }
